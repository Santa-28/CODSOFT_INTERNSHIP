...
class ChatBotApp:
    def __init__(self, root):
        ...
        # Sidebar frame
        self.sidebar_frame = ctk.CTkFrame(self.root, width=200, corner_radius=10, fg_color="#2c2c2c")
        self.sidebar_frame.grid(row=1, column=0, rowspan=2, sticky="nswe", padx=(20, 10), pady=20)
        self.sidebar_frame.grid_rowconfigure(4, weight=1)

        self.sidebar_label = ctk.CTkLabel(self.sidebar_frame, text="Options", font=("Montserrat", 16, "bold"), text_color="#ffffff")
        self.sidebar_label.grid(row=0, column=0, padx=20, pady=(20, 10))

        self.theme_label = ctk.CTkLabel(self.sidebar_frame, text="Change Theme:",font=("Montserrat", 13, "bold"), text_color="#cccccc")
        self.theme_label.grid(row=1, column=0, padx=20, pady=(10, 5))

        self.theme_option = ctk.CTkOptionMenu(self.sidebar_frame, values=["Light", "Dark"], command=self.change_theme, fg_color="#3e3e3e", button_color="#2c2c2c", text_color="#ffffff")
        self.theme_option.set("Dark")
        self.theme_option.grid(row=2, column=0, padx=20, pady=10)

        self.clear_button = ctk.CTkButton(self.sidebar_frame, text="Clear Chat", command=self.clear_chat, fg_color="#ff6666", hover_color="#ff4d4d")
        self.clear_button.grid(row=3, column=0, padx=20, pady=(10, 20))

        # Chat display area
        self.chat_display = ctk.CTkTextbox(self.root, width=560, height=400, wrap="word", corner_radius=10, fg_color="#3e3e3e", text_color="#ffffff", font=("Montserrat", 14,"bold"))
        ...
        self.send_button = ctk.CTkButton(self.input_frame, text="Send", command=self.send_message, fg_color="#4caf50", hover_color="#45a049")
        self.send_button.grid(row=0, column=1, padx=10, pady=10)

        self.user_input.bind("<Return>", lambda event: self.send_message())

        # Footer
        self.footer = ctk.CTkLabel(self.root, text="© 2024 Santa_Chatbot.",font=("Montserrat", 16, "bold"), text_color="#cccccc")
        self.footer.grid(row=3, column=0, columnspan=2, pady=(10, 20))
    ...
    def get_bot_response(self, message):
        ...
        if 'hello' in processed_input or 'hii' in processed_input:
